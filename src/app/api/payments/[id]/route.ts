...
import { validateUpdatePayment } from "./validators";
...
export async function GET(
  ...
  try {
    const result = await pool.query(
      `SELECT id,order_id, type, amount, status, transaction_id, paid_at, updated_by, created_at FROM "order".payments WHERE id = $1 AND is_deleted = false`,
      [id]
    );
...
export async function PUT(
  ...
  const updated_by = req.headers.get("x-payment-id") || body.updated_by;
  ...
  try {
    await pool.query(
      `UPDATE "order".payments SET ${fields} WHERE id = $${values.length + 1}`,
      [...values, id]
    );
